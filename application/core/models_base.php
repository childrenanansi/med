...
Abstract Class Model_Base {
    ...
    public function __construct($select = false, $tableName = 'task') {
        // объект бд коннекта
        global $dbObject;
        $this->db = $dbObject;
    ...
    public function save($table) {
    	...
    	if($table == 'medrab'){
    		$arrayAllFields = array_keys($this->fieldsTableMedrab());
    	}
            ...
            if(in_array("LIMIT", $allQuery)){
                foreach($select as $key => $val){
                    if(strtoupper($key) == "LIMIT"){
                        if(!empty($select['step'])) {
                            $querySql .= " LIMIT ". $select['step'].", " . $val;
                        }else{
                            $querySql .= " LIMIT " . $val;
                        }
                    }
    ...
    public function _getResultCount($sql){
        try{
            $db = $this->db;
            $stmt = $db->query($sql);
